...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U,  Log("imm %x", imm);R(rd) = s->pc + imm);
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S,  Log("target %x pc %x", src1 + imm, s->pc); Mw(src1 + imm, 4, src2));
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srli   , I, R(rd) = src1 >> imm);
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", srai   , I, R(rd) = (int32_t)src1 >> imm);
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (int64_t)(src1 * src2) >> 32);
