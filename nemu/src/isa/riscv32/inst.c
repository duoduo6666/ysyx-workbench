...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm; IFDEF(CONFIG_FTRACE, ftrace_check(s->pc, s->dnpc)));
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = src1 + imm; IFDEF(CONFIG_FTRACE, ftrace_check(s->pc, s->dnpc)));
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, s->dnpc = isa_raise_intr(R(MUXDEF(CONFIG_RVE, 15, 17)), s->pc));
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, if (rd!=0) R(rd) = csr(BITS(inst, 31, 20)); csr(BITS(inst, 31, 20)) = src1);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = csr(BITS(inst, 31, 20)); if (src1!=0) csr(BITS(inst, 31, 20)) = csr(BITS(inst, 31, 20)) | src1);
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, R(rd) = csr(BITS(inst, 31, 20)); if (src1!=0) csr(BITS(inst, 31, 20)) = csr(BITS(inst, 31, 20)) & ~src1);
  INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , I, if (rd!=0) R(rd) = csr(BITS(inst, 31, 20)); csr(BITS(inst, 31, 20)) = BITS(inst, 19, 15););
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , I, R(rd) = csr(BITS(inst, 31, 20)); csr(BITS(inst, 31, 20)) = csr(BITS(inst, 31, 20)) | BITS(inst, 19, 15));
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , I, R(rd) = csr(BITS(inst, 31, 20)); csr(BITS(inst, 31, 20)) = csr(BITS(inst, 31, 20)) & ~BITS(inst, 19, 15));
